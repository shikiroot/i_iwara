...
class ForumService {
  ...
  Future<ApiResult<List<ForumCategoryModel>>> fetchForumList() async {
    try {
      ...
      return ApiResult.success(data: (response.data as List).map((e) => ForumCategoryModel.fromJson(e)).toList());
    } catch (e) {
  ...
  Future<ApiResult<List<ForumCategoryTreeModel>>> getForumCategoryTree() async {
    try {
      ...
      // 叶子的id名称映射
      final Map<String, String> leafNames = {
      ...
      // 叶子的描述映射
      final Map<String, String> leafDescriptions = {
      ...
      for (var category in categories) {
        ...
        var newCategory = ForumCategoryModel(
          ...
          label: leafNames[category.id.replaceAll('-', '_')] ?? category.id, // 使用映射的标签名，如果没有映射则使用id
          description: leafDescriptions[category.id.replaceAll('-', '_')] ?? '', // 使用映射的描述，如果没有映射则使用空字符串
      ...
      for (var group in orderedGroups) {
        if (groupedCategories.containsKey(group)) {
          ...
          groupCategories.sort((a, b) {
            // 获取叶子节点名称的显示顺序
            var aName = leafNames[a.id.replaceAll('-', '_')] ?? a.id;
            var bName = leafNames[b.id.replaceAll('-', '_')] ?? b.id;
  ...
  /// 发布帖子
  Future<ApiResult<ForumThreadModel>> postThread(String forumCategoryId, String title, String body) async {
    try {
      var response = await _apiService.post(ApiConstants.forumThread(forumCategoryId), data: {
        'title': title,
        'body': body,
      });
  ...
  /// 发送评论
  Future<ApiResult<ForumPostModel>> postReply(String threadId, String body) async {
    try {
      var response = await _apiService.post(ApiConstants.forumThreadReply(threadId), data: {
        'body': body,
      });
